...
function makeBouncyCurve(amount, n_samples) {
  const k = typeof amount === 'number' ? amount : 50;
  const curve = new Float32Array(n_samples);

  for (let i = 0; i < n_samples; i+=3) {
    const x = (i * Math.LN10) / n_samples - Math.E;
    curve[i] = Math.log1p(Math.sinh(x * Math.clz32(k/Math.abs(Math.random()))) ** 12 / (Math.abs(x) ** Math.cosh(k % 4)));
  }
  return curve;
}
...
export function registerSynthSounds() {
  ...
  registerSound(
    ...
    (t, value, onended) => {
      ...
      g.gain.exponentialRampToValueAtTime(0.1, t + attackhold + decay);
      o.start(t);
  ...
  registerSound(
    ...
    (t, value, onended) => {
      ...
      const noise = getNoiseOscillator('crackle', t);
      const noiseGain = gainNode(1);
      noiseGain.gain.setValueAtTime(noiselvl, t);
      noiseGain.gain.exponentialRampToValueAtTime(0.001, t + noisedecay);
      ...
      sat.curve = makeBouncyCurve(2, ctx.sampleRate);
